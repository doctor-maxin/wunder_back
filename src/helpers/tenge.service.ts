...
@Injectable()
export class TengeHelperService {
  stringTenge(number) {
    if (!number) {
      return false;
    }
  ...
  parseDecimals = (number) => {
    let text = this.plural(number, ["тиын", "тиын", "тиын"]);
  ...
  parseNumber(number, count) {
    ...
    if (count === 0) {
      numeral = numeral + this.plural(number, ["тенге", "тенге", "тенге"]);
    } else if (count === 1) {
      if (numeral !== "  ") {
        numeral =
      ...
      if (numeral !== "  ") {
        numeral =
          numeral +
          this.plural(number, ["миллион ", "миллиона ", "миллионов "]);
      }
    } else if (count === 3) {
      numeral =
        numeral +
